# CybrScrape Extract Command Guide

**Web Scraping through the terminal without requiring any programming!**

The `cybrscrape extract` command lets you download and extract content from websites directly from your terminal without writing any code. Ideal for beginners, researchers, and anyone requiring rapid web data extraction.
...
- **Basic Website Download**
    ...
    ```bash
    cybrscrape extract get "https://example.com" page_content.txt
    ```
...
- **Save as Different Formats**
    ...
    cybrscrape extract get "https://blog.example.com" article.md
    
    # Save the HTML content as it is to the file
    cybrscrape extract get "https://example.com" page.html
    
    # Save a clean version of the text content of the webpage to the file
    cybrscrape extract get "https://example.com" content.txt
  
    # Or use the Docker image with something like this:
    docker run -v $(pwd)/output:/output cybrscrape extract get "https://blog.example.com" /output/article.md 
    ```
...
You can display the available commands through `cybrscrape extract --help` to get the following list:
```bash
Usage: cybrscrape extract [OPTIONS] COMMAND [ARGS]...
...
1. **GET Request**
    ...
    ```bash
    cybrscrape extract get [URL] [OUTPUT_FILE] [OPTIONS]
    ```
    
    **Examples:**
    ```bash
    # Basic download
    cybrscrape extract get "https://news.site.com" news.md
    
    # Download with custom timeout
    cybrscrape extract get "https://example.com" content.txt --timeout 60
    
    # Extract only specific content using CSS selectors
    cybrscrape extract get "https://blog.example.com" articles.md --css-selector "article"
   
    # Send a request with cookies
    cybrscrape extract get "https://cybrscrape.requestcatcher.com" content.md --cookies "session=abc123; user=john"
   
    # Add user agent
    cybrscrape extract get "https://api.site.com" data.json -H "User-Agent: MyBot 1.0"
    
    # Add multiple headers
    cybrscrape extract get "https://site.com" page.html -H "Accept: text/html" -H "Accept-Language: en-US"
    ```
    Get the available options for the command with `cybrscrape extract get --help` as follows:
    ```bash
    Usage: cybrscrape extract get [OPTIONS] URL OUTPUT_FILE
...
2. **Post Request**
    ...
    ```bash
    cybrscrape extract post [URL] [OUTPUT_FILE] [OPTIONS]
    ```
    
    **Examples:**
    ```bash
    # Submit form data
    cybrscrape extract post "https://api.site.com/search" results.html --data "query=python&type=tutorial"
    
    # Send JSON data
    cybrscrape extract post "https://api.site.com" response.json --json '{"username": "test", "action": "search"}'
    ```
    Get the available options for the command with `cybrscrape extract post --help` as follows:
    ```bash
    Usage: cybrscrape extract post [OPTIONS] URL OUTPUT_FILE
...
3. **Put Request**
    ...
    ```bash
    cybrscrape extract put [URL] [OUTPUT_FILE] [OPTIONS]
    ```
    
    **Examples:**
    ```bash
    # Send data
    cybrscrape extract put "https://cybrscrape.requestcatcher.com/put" results.html --data "update=info" --impersonate "firefox"
    
    # Send JSON data
    cybrscrape extract put "https://cybrscrape.requestcatcher.com/put" response.json --json '{"username": "test", "action": "search"}'
    ```
    Get the available options for the command with `cybrscrape extract put --help` as follows:
    ```bash
    Usage: cybrscrape extract put [OPTIONS] URL OUTPUT_FILE
...
4. **Delete Request**
    ...
    ```bash
    cybrscrape extract delete [URL] [OUTPUT_FILE] [OPTIONS]
    ```
    
    **Examples:**
    ```bash
    # Send data
    cybrscrape extract delete "https://cybrscrape.requestcatcher.com/delete" results.html
    
    # Send JSON data
    cybrscrape extract delete "https://cybrscrape.requestcatcher.com/" response.txt --impersonate "chrome"
    ```
    Get the available options for the command with `cybrscrape extract delete --help` as follows:
    ```bash
    Usage: cybrscrape extract delete [OPTIONS] URL OUTPUT_FILE
...
1. **fetch - Handle Dynamic Content**
    ...
    ```bash
    cybrscrape extract fetch [URL] [OUTPUT_FILE] [OPTIONS]
    ```
    
    **Examples:**
    ```bash
    # Wait for JavaScript to load content and finish network activity
    cybrscrape extract fetch "https://cybrscrape.requestcatcher.com/" content.md --network-idle
    
    # Wait for specific content to appear
    cybrscrape extract fetch "https://cybrscrape.requestcatcher.com/" data.txt --wait-selector ".content-loaded"
    
    # Run in visible browser mode (helpful for debugging)
    cybrscrape extract fetch "https://cybrscrape.requestcatcher.com/" page.html --no-headless --disable-resources
    ```
    Get the available options for the command with `cybrscrape extract fetch --help` as follows:
    ```bash
    Usage: cybrscrape extract fetch [OPTIONS] URL OUTPUT_FILE
...
2. **stealthy-fetch - Bypass Protection**
    ...
    ```bash
    cybrscrape extract stealthy-fetch [URL] [OUTPUT_FILE] [OPTIONS]
    ```
    
    **Examples:**
    ```bash
    # Bypass basic protection
    cybrscrape extract stealthy-fetch "https://cybrscrape.requestcatcher.com" content.md
    
    # Solve Cloudflare challenges
    cybrscrape extract stealthy-fetch "https://nopecha.com/demo/cloudflare" data.txt --solve-cloudflare --css-selector "#padded_content a"
    
    # Use a proxy for anonymity.
    cybrscrape extract stealthy-fetch "https://site.com" content.md --proxy "http://proxy-server:8080"
    ```
    Get the available options for the command with `cybrscrape extract stealthy-fetch --help` as follows:
    ```bash
    Usage: cybrscrape extract stealthy-fetch [OPTIONS] URL OUTPUT_FILE
