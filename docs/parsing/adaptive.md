...
Adaptive scraping (previously known as automatch) is one of CybrScrape's most powerful features. It allows your scraper to survive website changes by intelligently tracking and relocating elements.
...
```
The selector will no longer function, and your code needs maintenance. That's where CybrScrape's `adaptive` feature comes into play.

With CybrScrape, you can enable the `adaptive` feature the first time you select an element, and the next time you select that element and it doesn't exist, CybrScrape will remember its properties and search on the website for the element with the highest percentage of similarity to that element, and without AI :)

```python
from cybrscrape import Selector, Fetcher
# Before the change
...
if not element:  # One day website changes?
    element = page.css('#p1', adaptive=True)  # CybrScrape still finds it!
...
...    print('CybrScrape found the same element in the old and new designs!')
'CybrScrape found the same element in the old and new designs!'
```
Note that I introduced a new argument called `adaptive_domain`. This is because, for CybrScrape, these are two different domains (`archive.org` and `stackoverflow.com`), so CybrScrape will isolate their `adaptive` data. To inform CybrScrape that they are the same website, we must pass the custom domain we wish to use while saving `adaptive` data for both, ensuring CybrScrape doesn't isolate them.
...
!!! info
    ...
    The main reason for creating the `adaptive_domain` argument was to handle if the website changed its URL while changing the design/structure. In that case, you can use it to continue using the previously stored adaptive data for the new URL. Otherwise, cybrscrape will consider it a new website and discard the old data.
...
With as few technical details as possible, the general logic goes as follows:
  ...
  1. You tell CybrScrape to save that element's unique properties in one of the ways we will show below.
  2. CybrScrape uses its configured database (SQLite by default) and saves each element's unique properties.
...
For CybrScrape, the unique elements we are relying on are:
...
>>> page = Fetcher.get('https://example.com')
```
If you are using the [Selector](main_classes.md#selector) class, you need to pass the url of the website you are using with the argument `url` so CybrScrape can separate the properties saved for each element by domain.
