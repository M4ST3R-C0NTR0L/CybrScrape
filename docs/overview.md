...
## Fetching websites
Instead of passing the raw HTML to CybrScrape, you can retrieve a website's response directly via HTTP requests or by fetching it in a browser.
...
page = Fetcher.get('https://cybrscrape.requestcatcher.com/get', impersonate="chrome")
```
...
>>> page = Fetcher.post('https://cybrscrape.requestcatcher.com/post', data={'key': 'value'}, proxy='http://username:password@localhost:8030')
>>> page = Fetcher.put('https://cybrscrape.requestcatcher.com/put', data={'key': 'value'})
>>> page = Fetcher.delete('https://cybrscrape.requestcatcher.com/delete')
```
...
>>> page = await AsyncFetcher.post('https://cybrscrape.requestcatcher.com/post', data={'key': 'value'}, proxy='http://username:password@localhost:8030')
>>> page = await AsyncFetcher.put('https://cybrscrape.requestcatcher.com/put', data={'key': 'value'})
>>> page = await AsyncFetcher.delete('https://cybrscrape.requestcatcher.com/delete')
```
...
>>> page = DynamicFetcher.fetch('https://www.google.com/search?q=%22CybrScrape%22', disable_resources=True)  # Vanilla Playwright option
>>> page.css("#search a::attr(href)").get()
'https://github.com/M4ST3R-C0NTR0L/CybrScrape'
>>> # The async version of fetch
>>> page = await DynamicFetcher.async_fetch('https://www.google.com/search?q=%22CybrScrape%22', disable_resources=True)
>>> page.css("#search a::attr(href)").get()
'https://github.com/M4ST3R-C0NTR0L/CybrScrape'
```
...
That's CybrScrape at a glance. If you want to learn more, continue to the next section.