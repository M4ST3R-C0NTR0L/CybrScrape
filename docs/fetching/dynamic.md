...
CybrScrape provides many options with this fetcher and its session classes. To make it as simple as possible, we will list the options here and give examples of how to use most of them.
...
```python
from cybrscrape.fetchers import DynamicSession, ProxyRotator
...
```python
page = DynamicFetcher.fetch('https://raw.githubusercontent.com/D4Vinci/CybrScrape/main/images/main_cover.png')
...
```python
from cybrscrape.fetchers import DynamicSession
...
```python
import asyncio
from cybrscrape.fetchers import AsyncDynamicSession
