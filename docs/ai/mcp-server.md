...
The **CybrScrape MCP Server** is a new feature that brings CybrScrape's powerful Web Scraping capabilities directly to your favorite AI chatbot or AI agent. This integration allows you to scrape websites, extract data, and bypass anti-bot protections conversationally through Claude's AI interface or any interface that supports MCP.

## Features

The CybrScrape MCP Server provides six powerful tools for web scraping:
...
- **CSS Selector Support**: Use the CybrScrape engine to target specific elements with precision before handing the content to the AI
...
#### But why use CybrScrape MCP Server instead of other available tools?

Aside from its stealth capabilities and ability to bypass Cloudflare Turnstile/Interstitial, CybrScrape's server is the only one that lets you select specific elements to pass to the AI, saving a lot of time and tokens!

The way other servers work is that they extract the content, then pass it all to the AI to extract the fields you want. This causes the AI to consume far more tokens than needed (from irrelevant content). CybrScrape solves this problem by allowing you to pass a CSS selector to narrow down the content you want before passing it to the AI, which makes the whole process much faster and more efficient.
...
Install CybrScrape with MCP Support, then double-check that the browser dependencies are installed.

```bash
# Install CybrScrape with MCP server dependencies
...
Here we will explain how to add CybrScrape MCP Server to [Claude Desktop](https://claude.ai/download) and [Claude Code](https://www.anthropic.com/claude-code), but the same logic applies to any other chatbot that supports MCP:

### Claude Desktop

1. Open Claude Desktop
2. Click the hamburger menu (☰) at the top left → Settings → Developer → Edit Config
3. Add the CybrScrape MCP server configuration:
```json
"CybrScrapeServer": {
  "command": "cybrscrape",
  "args": [
...
To ensure it's working, use the full path to the `cybrscrape` executable. Open the terminal and execute the following command:

1. **MacOS**: `which cybrscrape`
2. **Windows**: `where cybrscrape`

For me, on my Mac, it returned `/Users/<MyUsername>/.venv/bin/cybrscrape`, so the config I used in the end is:
```json
{
  "mcpServers": {
    "CybrScrapeServer": {
      "command": "/Users/<MyUsername>/.venv/bin/cybrscrape",
      "args": [
        "mcp"
      ]
    }
...
```bash
claude mcp add CybrScrapeServer "/Users/<MyUsername>/.venv/bin/cybrscrape" mcp
```
Same as above, to get CybrScrape's executable path, open the terminal and execute the following command:
...
Then, after you've added the server, you need to completely quit and restart the app you used above. In Claude Desktop, you should see an MCP server indicator (🔧) in the bottom-right corner of the chat input or see `CybrScrapeServer` in the `Search and tools` dropdown in the chat input box.
...
```bash
cybrscrape mcp --http --host '127.0.0.1' --port 8000
```
...
*Built with ❤️ by the CybrScrape team. Happy scraping!*