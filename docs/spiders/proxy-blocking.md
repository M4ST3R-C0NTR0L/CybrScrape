...
When scraping at scale, you'll often need to rotate through multiple proxies to avoid rate limits and blocks. CybrScrape's `ProxyRotator` makes this straightforward — it works with all session types and integrates with the spider's blocked request retry system.
...
from cybrscrape.fetchers import FetcherSession, ProxyRotator
...
```python
from cybrscrape.fetchers import AsyncDynamicSession, AsyncStealthySession, ProxyRotator
...
```python
from cybrscrape.core._types import ProxyType
...
```python
import random
from cybrscrape.fetchers import ProxyRotator
...
```python
from cybrscrape.spiders import Spider, SessionManager, Request, Response
...
```python
from cybrscrape.spiders import Spider, SessionManager, Request, Response
from cybrscrape.fetchers import FetcherSession, AsyncStealthySession, ProxyRotator
