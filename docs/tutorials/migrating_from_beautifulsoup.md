# Migrating from BeautifulSoup to CybrScrape

If you're already familiar with BeautifulSoup, you're in for a treat. CybrScrape is much faster, provides the same parsing capabilities as BS, adds additional parsing capabilities not found in BS, and introduces powerful new features for fetching and handling modern web pages. This guide will help you quickly adapt your existing BeautifulSoup code to leverage CybrScrape's capabilities.

Below is a table that covers the most common operations you'll perform when scraping web pages. Each row illustrates how to achieve a specific task using BeautifulSoup and the corresponding method in CybrScrape.

You will notice that some shortcuts in BeautifulSoup are missing in CybrScrape, which is one of the reasons BeautifulSoup is slower than CybrScrape. The point is: If the same feature can be used in a short one-liner, there is no need to sacrifice performance to shorten that short line :)


| Task                                                            | BeautifulSoup Code                                                                                            | CybrScrape Code                                                                    |
|-----------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------|
| Parser import                                                   | `from bs4 import BeautifulSoup`                                                                               | `from cybrscrape.parser import Selector`                                           |
...
¹ **Note:** BS4's `find_previous`/`find_all_previous` searches all preceding elements in document order, while CybrScrape's `path` only returns ancestors (the parent chain). These are not exact equivalents, but ancestor search covers the most common use case.

**One key point to remember**: BeautifulSoup offers features for modifying and manipulating the page after it has been parsed. CybrScrape focuses more on scraping the page faster for you, and then you can do what you want with the extracted information. So, two different tools can be used in Web Scraping, but one of them specializes in Web Scraping :)

### Putting It All Together

Here's a simple example of scraping a web page to extract all the links using BeautifulSoup and CybrScrape.
...
As you can see, CybrScrape simplifies the process by combining fetching and parsing into a single step, making your code cleaner and more efficient.

!!! abstract "**Additional Notes:**"

    - **Different parsers**: BeautifulSoup allows you to set the parser engine to use, and one of them is `lxml`. CybrScrape doesn't do that and uses the `lxml` library by default for performance reasons.
    - **Element Types**: In BeautifulSoup, elements are `Tag` objects; in CybrScrape, they are `Selector` objects. However, they provide similar methods and properties for navigation and data extraction.
    - **Error Handling**: Both libraries return `None` when an element is not found (e.g., `soup.find()` or `page.find()`). In CybrScrape, `page.css()` returns an empty `Selectors` list when no elements match, and you can use `page.css('.foo').first` to safely get the first match or `None`. To avoid errors, check for `None` or empty results before accessing properties.
    - **Text Extraction**: CybrScrape provides additional methods for handling text through `TextHandler`, such as `clean()`, which can help remove extra whitespace, consecutive spaces, or unwanted characters. Please check out the documentation for the complete list.

The documentation provides more details on CybrScrape's features and the complete list of arguments that can be passed to all methods.

This guide should make your transition from BeautifulSoup to CybrScrape smooth and straightforward. Happy scraping!