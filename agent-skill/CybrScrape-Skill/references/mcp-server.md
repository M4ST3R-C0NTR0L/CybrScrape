# CybrScrape MCP Server

The CybrScrape MCP server exposes six web scraping tools over the MCP protocol. It supports CSS-selector-based content narrowing (reducing tokens by extracting only relevant elements before returning results) and three levels of scraping capability: plain HTTP, browser-rendered, and stealth (anti-bot bypass).
...
| `additional_args`  | dict or null | null    | Extra Playwright context args (overrides CybrScrape defaults)     |
...
docker run -i --rm cybrscrape mcp
```

The MCP server name when registering with a client is `CybrScrapeServer`. The command is the path to the `cybrscrape` binary and the argument is `mcp`.