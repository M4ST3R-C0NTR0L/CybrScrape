...
CybrScrape's spider system is an async crawling framework designed for concurrent, multi-session crawls with built-in pause/resume support. It brings together CybrScrape's parsing engine and fetchers into a unified crawling API while adding scheduling, concurrency control, and checkpointing.
...
```python
from cybrscrape.spiders import Spider, Response, Request
...
If you're coming from Scrapy, here's how CybrScrape's spider system maps:

| Concept            | Scrapy                        | CybrScrape                                                       |
|--------------------|-------------------------------|-----------------------------------------------------------------|
| Spider definition  | `scrapy.Spider` subclass      | `cybrscrape.spiders.Spider` subclass                             |
