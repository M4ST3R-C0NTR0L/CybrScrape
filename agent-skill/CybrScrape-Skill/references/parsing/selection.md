# Querying elements
CybrScrape currently supports parsing HTML pages exclusively (no XML feeds), because the adaptive feature does not work with XML.
...
There are also other indirect ways to find elements. CybrScrape can also find elements similar to a given element; see [Finding Similar Elements](#finding-similar-elements).
...
The logic follows the same conventions as Scrapy/Parsel. However, CybrScrape does not implement the XPath extension function `has-class` as Scrapy/Parsel does. Instead, it provides the `has_class` method on returned elements.
...
CybrScrape provides two ways to select elements based on their direct text content:
...
CybrScrape can find elements similar to a given element, inspired by the AutoScraper library but usable with elements found by any method.

Given an element (e.g., a product found by title), calling `.find_similar()` on it causes CybrScrape to:
...
3. As a final check, CybrScrape uses fuzzy matching to drop elements whose attributes don't resemble the original element's attributes. A configurable percentage controls this step (see arguments below).
...
'//body/div/div[2]/div/div/span[2]/a'
```
**Note:** When generating a short selector, CybrScrape tries to find a unique element (e.g., one with an `id` attribute) as a stop point. If none exists, the short and full selectors will be identical.
