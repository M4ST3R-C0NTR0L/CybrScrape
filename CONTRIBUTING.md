# Contributing to CybrScrape

Thank you for your interest in contributing to CybrScrape! 

Everybody is invited and welcome to contribute to CybrScrape. 
...
There are many ways to contribute to CybrScrape. Here are some of them:

- Report bugs and request features using the [GitHub issues](https://github.com/M4ST3R-C0NTR0L/CybrScrape/issues). Please follow the issue template to help us resolve your issue quickly.
- Blog about CybrScrape. Tell the world how you're using CybrScrape. This will help newcomers with more examples and increase the CybrScrape project's visibility.
- Join the [Discord community](https://discord.gg/EMgGbDceNQ) and share your ideas on how to improve CybrScrape. We're always open to suggestions.
- If you are not a developer, perhaps you would like to help with translating the [documentation](https://github.com/M4ST3R-C0NTR0L/CybrScrape/tree/docs)?
...
- **The PR must be made against the [**dev**](https://github.com/M4ST3R-C0NTR0L/CybrScrape/tree/dev) branch of CybrScrape. Any PR made against the main branch will be rejected.**
...
If you have decided to make a contribution to CybrScrape, but you do not know what to contribute, here are some ways to find pending work:

- Check out the [contribution](https://github.com/M4ST3R-C0NTR0L/CybrScrape/contribute) GitHub page, which lists open issues tagged as `good first issue`. These issues provide a good starting point.
- There are also the [help wanted](https://github.com/M4ST3R-C0NTR0L/CybrScrape/issues?q=is%3Aissue%20label%3A%22help%20wanted%22%20state%3Aopen) issues, but know that some may require familiarity with the CybrScrape code base first. You can also target any other issue, provided it is not tagged as `invalid`, `wontfix`, or similar tags.
...
## Coding style
Please follow these coding conventions as we do when writing code for CybrScrape:
...
1. Fork the repository and clone your fork:
   ```bash
   git clone https://github.com/<your-username>/CybrScrape.git
   cd CybrScrape
   git checkout dev
   ```
...
Setting the cybrscrape logging level to `debug` makes it easier to know what's happening in the background.
```python
import logging
...
```commandline
pip3 install git+https://github.com/M4ST3R-C0NTR0L/CybrScrape.git@dev
```

## Tests
CybrScrape includes a comprehensive test suite that can be executed with pytest. However, first, you need to install all libraries and `pytest-plugins` listed in `tests/requirements.txt`. Then, running the tests will result in an output like this:
   ```bash
   $ pytest tests -n auto
   =============================== test session starts ===============================
   platform darwin -- Python 3.13.8, pytest-8.4.2, pluggy-1.6.0 -- /Users/<redacted>/.venv/bin/python3.13
   cachedir: .pytest_cache
   rootdir: /Users/<redacted>/cybrscrape
